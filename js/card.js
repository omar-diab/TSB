...
function renderCart() {
  ...
  if (cart.length === 0) {
    cartContainer.innerHTML = `
      <p class="pt-14 text-center text-3xl font-semibold text-[var(--heading-color)] dark:text-[var(--heading-color)]">
        Your cart is empty.
      </p>
    `;
    return;
  ...
  cartContainer.innerHTML = `
    <h2 class="text-3xl font-bold mb-6 text-[var(--heading-color)] dark:text-[var(--heading-color)]">
      Your Cart
    </h2>
  `;
  ...
  cart.forEach((product, index) => {
    ...
    productDiv.innerHTML = `
      <img 
        src="${product.image && product.image !== 'image' ? product.image : 'https://via.placeholder.com/80'}" 
        alt="${product.name}" 
...
// Use event delegation for better performance on dynamic content
